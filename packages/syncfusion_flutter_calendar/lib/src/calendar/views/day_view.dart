...
class _TimeSlotRenderObject extends CustomCalendarRenderObject {
  ...
  void _drawTimeSlots(Canvas canvas, int visibleDatesCount) {
    ...
    for (int i = 1; i <= horizontalLinesCount; i++) {
      double columnStartX = startXPosition;
      for (int col = 0; col < visibleDatesCount; col++) {
        final double columnEndX = columnStartX + _cellWidth;
        canvas.drawLine(
          Offset(columnStartX, y),
          Offset(columnEndX, y),
          _linePainter,
        );
    ...
    _linePainter.isAntiAlias = false;
